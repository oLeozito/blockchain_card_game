...
// Variáveis globais para a Blockchain
var (
	blockchainClient *ethclient.Client
	gameContract     *contracts.Contracts
	authTransactor   *bind.TransactOpts
)

func initBlockchain() {
	clientUrl := "http://blockchain:8545"
	var err error

	// --- TENTATIVA DE CONEXÃO COM RETRY (NOVO) ---
	// Tenta conectar por até 30 segundos
	for i := 0; i < 10; i++ {
		blockchainClient, err = ethclient.Dial(clientUrl)
		if err == nil {
			// Tenta um ping real (ChainID) para garantir que não é só TCP aberto
			_, err = blockchainClient.ChainID(context.Background())
			if err == nil {
				break // Sucesso total! Sai do loop
			}
		}
		logger.Printf(ColorYellow+"Tentando conectar na Blockchain (%d/10)... Erro: %v"+ColorReset, i+1, err)
		time.Sleep(3 * time.Second)
	}

	if err != nil {
		logger.Printf(ColorRed + "FALHA CRÍTICA: Desistindo da Blockchain após várias tentativas." + ColorReset)
		return
	}
	logger.Printf(ColorGreen+"Conectado e Sincronizado com o nó Ethereum em %s"+ColorReset, clientUrl)
	// ---------------------------------------------

	// Chave Privada de Teste do Hardhat (Conta #0)
	privateKeyHex := "ac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"
	privateKey, err := crypto.HexToECDSA(privateKeyHex)
	if err != nil {
		logger.Fatalf("Erro ao carregar chave privada: %v", err)
	}

	publicKey := privateKey.Public()
	publicKeyECDSA, ok := publicKey.(*ecdsa.PublicKey)
	if !ok {
		logger.Fatal("Erro ao converter chave pública")
	}
	fromAddress := crypto.PubkeyToAddress(*publicKeyECDSA)

	// Configura o Transactor
	chainId, err := blockchainClient.ChainID(context.Background())
	if err != nil {
		logger.Printf(ColorRed+"Erro ao obter ChainID: %v"+ColorReset, err)
		return
	}
	...
	authTransactor.Value = big.NewInt(0)
	authTransactor.GasLimit = uint64(3000000)
	authTransactor.GasPrice = big.NewInt(875000000)

	// DEPLOY DO CONTRATO
	logger.Println("Tentando fazer deploy do contrato GameAssets...")
	address, tx, instance, err := contracts.DeployContracts(authTransactor, blockchainClient)
	if err != nil {
		logger.Printf(ColorRed+"Erro no Deploy: %v"+ColorReset, err)
		return
	}

	gameContract = instance
	logger.Printf(ColorGreen + "CONTRATO DEPLOYADO COM SUCESSO!" + ColorReset)
	logger.Printf("Endereço do Contrato: %s", address.Hex())
	logger.Printf("Transação: %s", tx.Hash().Hex())
	logger.Printf("Dono do contrato: %s", fromAddress.Hex())
}

// --- CARTEIRA DETERMINÍSTICA ---
// Gera sempre a mesma carteira para o mesmo login.
// Isso garante que todos os nós do Raft cheguem ao mesmo endereço.
func generateDeterministicWallet(login string) (string, string) {
	// 1. Cria uma "semente" baseada no login + um segredo do sistema
	// (Isso substitui a aleatoriedade pura)
	seedText := fmt.Sprintf("PBL_REDES_SECRET_SEED_%s", login)
	
	// 2. Gera um Hash dessa semente (sempre igual para o mesmo texto)
	hash := crypto.Keccak256([]byte(seedText))
	
	// 3. Transforma o Hash em Chave Privada
	privateKey, err := crypto.ToECDSA(hash)
	if err != nil {
		// Fallback de emergência (não deve acontecer com Keccak256)
		logger.Printf("Erro ao gerar chave determinística: %v", err)
		return "", ""
	}

	// 4. Deriva o endereço público
	publicKey := privateKey.Public()
	publicKeyECDSA, _ := publicKey.(*ecdsa.PublicKey)
	address := crypto.PubkeyToAddress(*publicKeyECDSA).Hex()
	
	privKeyHex := hexutil.Encode(crypto.FromECDSA(privateKey))
...
func (f *FSM) Apply(log *raft.Log) interface{} {
	...
	case "CADASTRAR_JOGADOR":
		...
		// GERA A WALLET AQUI
        walletAddr, privKey := generateDeterministicWallet(data.Login)
        
        players[data.Login] = &User{
            Login:         data.Login,
            Senha:         data.Senha,
            Moedas:        100,
            WalletAddress: walletAddr, // Salva o endereço
        }

		logger.Printf("[FSM] Jogador %s cadastrado. Wallet: %s (PrivKey: %s)", data.Login, walletAddr, privKey)
	...
	case "COMPRAR_PACOTE":
		...
		// --- INTEGRAÇÃO BLOCKCHAIN (NOVO TRECHO) ---
		// Tenta registrar a carta na blockchain. Se falhar, loga o erro mas não trava o jogo.
		// ATENÇÃO: Isso deve ser feito numa goroutine para não bloquear o Raft,
		// MAS idealmente o líder faria isso fora da FSM. Para o teste de amanhã,
		// vamos colocar aqui mas sabendo que é uma "simplificação".

		go func(playerAddr string, cardID string, rarity string) {
			// 1. Verificação de Segurança Básica
			if gameContract == nil || authTransactor == nil { return }

			// 2. FILTRO DE LÍDER (AQUI ESTÁ A MÁGICA)
			// Se eu NÃO sou o líder, eu não mando pra blockchain.
			// Eu apenas atualizei minha memória RAM (acima) e fico quieto.
			...
			idHash := big.NewInt(0)
			hashBytes := crypto.Keccak256([]byte(cardID))
			idHash.SetBytes(hashBytes)

			// 4. Verificação de Existência (Idempotência)
			// Mesmo sendo líder, verifico se já não fiz isso antes (em caso de restart)
			cartaExistente, errCheck := gameContract.Cards(nil, idHash)
			if errCheck == nil && cartaExistente.Id.Cmp(big.NewInt(0)) != 0 {
				return // Já existe
			}
			...
			if playerAddr != "" {
				destAddress = common.HexToAddress(playerAddr)
			} else {
				destAddress = common.HexToAddress("0x70997970C51812dc3A010C7d01b50e0d17dc79C8")
			}

			logger.Printf(ColorYellow+"[Líder] Mintando carta %s na Blockchain..."+ColorReset, cardID)

			// 6. Envia Transação
			tx, err := gameContract.MintCard(authTransactor, idHash, rarity, destAddress)
			if err != nil {
				logger.Printf(ColorRed+"Erro Blockchain: %v"+ColorReset, err)
			} else {
				logger.Printf(ColorGreen+"SUCESSO! Tx Enviada: %s"+ColorReset, tx.Hash().Hex())
			}
		}(player.WalletAddress, newInstrument.ID, newInstrument.Rarity)
		// ---------------------------------------------
	...
	case "LOGIN_RAFT":
		...
		// Caso o player seja antigo e nao tenha uma wallet ja.
		if player.WalletAddress == "" {
            wAddr, pKey := generateDeterministicWallet(player.Login)
            player.WalletAddress = wAddr
            logger.Printf("[FSM] Wallet gerada para veterano %s: %s (Key: %s)", player.Login, wAddr, pKey)
        }
...
func logoutUser(playerLogin string) {
	...
	if raftNode.State() != raft.Leader {
		...
		if leaderRaftAddr == "" {
			return
		} // Se não sabe o líder, não pode deslogar
		leaderHttpAddr := strings.Replace(leaderRaftAddr, ":12000", ":8090", 1)
...
func reconnectSession(conn net.Conn, playerLogin string) string { // Retorna o status ("RECONNECTED_OK" ou erro)
	...
	if raftNode.State() != raft.Leader {
		...
		resp, err := client.Post(reqURL, "application/json", bytes.NewBuffer(postBody))
		if err != nil {
			return "ERROR_CONNECTING_LEADER"
		}
		defer resp.Body.Close()
...
func determineWinnerAndUpdateCoinsFSM(sala *Sala, p1 *User, p2 *User) string {
	...
	// Lógica de Vencedor (Off-Chain / Memória)
	...
	// Atualiza saldo na memória (Raft - Todos executam)
	p1.Moedas += p1CoinsEarned
	p2.Moedas += p2CoinsEarned

	// --- INTEGRAÇÃO BLOCKCHAIN (REGISTRO DE PARTIDA) ---
	// Lançamos numa thread separada para não travar o Raft
	go func(winnerLogin string, user1, user2 *User, score1, score2 int) {
		...
		// 2. FILTRO DE LÍDER (AQUI ESTÁ A MÁGICA)
		// Se eu NÃO sou o líder, eu paro aqui. Não mando nada pra blockchain.
		// Isso garante que apenas 1 transação seja enviada, evitando blocos duplicados.
		if raftNode.State() != raft.Leader {
			return
		}

		// 3. Preparação dos Endereços (Quem é quem na blockchain?)
		var wWallet, lWallet string
		var sWin, sLose int

		// Identifica carteiras baseadas em quem ganhou
		if winnerLogin == user1.Login {
			wWallet = user1.WalletAddress
			lWallet = user2.WalletAddress
			sWin = score1
			sLose = score2
		} else if winnerLogin == user2.Login {
			wWallet = user2.WalletAddress
			lWallet = user1.WalletAddress
			sWin = score2
			sLose = score1
		} else {
			// Empate: Tratamos P1 como "vencedor" técnico pro registro, mas scores iguais
			wWallet = user1.WalletAddress
			lWallet = user2.WalletAddress
			sWin = score1
			sLose = score2
		...
		// Fallback se não tiver wallet (testes antigos)
		if wWallet == "" { wWallet = "0x70997970C51812dc3A010C7d01b50e0d17dc79C8" }
		if lWallet == "" { lWallet = "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC" }

		// 4. Conversão para Tipos Ethereum
		addrWinner := common.HexToAddress(wWallet)
		addrLoser := common.HexToAddress(lWallet)
		bigScoreWin := big.NewInt(int64(sWin))
		bigScoreLose := big.NewInt(int64(sLose))

		logger.Printf(ColorYellow+"[Líder] Registrando partida na Blockchain... Vencedor: %s (%d x %d)"+ColorReset, winnerLogin, sWin, sLose)

		// 5. Envia a Transação (RecordMatch)
		// Certifique-se que o GameAssets.go foi atualizado com 'abigen' para ter essa função!
		tx, err := gameContract.RecordMatch(authTransactor, addrWinner, addrLoser, bigScoreWin, bigScoreLose)
		if err != nil {
			logger.Printf(ColorRed+"Erro Blockchain: %v"+ColorReset, err)
		} else {
			logger.Printf(ColorGreen+"SUCESSO! Partida Registrada. Tx: %s"+ColorReset, tx.Hash().Hex())
		}

	}(winner, p1, p2, game.Player1Score, game.Player2Score)
	// ---------------------------------------------------
...
func findRoom(conn net.Conn, mode string, roomCode string) {
	...
	if player.SelectedInstrument == nil {
		sendScreenMsg(conn, "Selecione um instrumento primeiro!")
		return
	}
		...
		if mode == "PUBLIC" {
			...
			if waitingRoomExists {
				...
				if err := applyFuture.Error(); err != nil {
					logger.Printf(ColorRed+"Erro RAFT ao criar sala pública para %s: %v"+ColorReset, player.Login, err)
					sendScreenMsg(conn, "Erro RAFT ao criar sala.")
					return
				}
				fsmResp := applyFuture.Response().(fsmApplyResponse)
				if fsmResp.err != nil || fsmResp.response != "RoomCreated" {
					logger.Printf(ColorYellow+"Falha FSM ao criar sala pública para %s: %v %v"+ColorReset, player.Login, fsmResp.err, fsmResp.response)
					sendScreenMsg(conn, fmt.Sprintf("Erro ao criar sala: %v", fsmResp.response))
					return
				}
				...
				if ok {
					...
					logger.Printf(ColorRed+"Erro CRÍTICO: Sala pública %s criada pela FSM mas não encontrada!"+ColorReset, roomID)
				}
				mu.Unlock()
	...
	case map[string]string: // Sucesso - Jogo Iniciado
		if status, ok := result["status"]; ok && status == "GameStarted" {
			...
			if exists {
				...
				if sala.P1Login == p1Login && p1ok && p1.Online {
					sala.Jogador1 = p1.Conn
				}
				if sala.P2Login == p2Login && p2ok && p2.Online {
					sala.Jogador2 = p2.Conn
				}
...
func startGameAndNotify(sala *Sala) {
	if raftNode.State() != raft.Leader {
		return
	} // Apenas o líder notifica via MQTT
...
func handlePlayNote(conn net.Conn, data interface{}) {
	...
	if raftNode.State() != raft.Leader {
		...
		case map[string]interface{}: // Sucesso (NotePlayed ou GameOver)
			...
			if salaID == "" {
				logger.Printf(ColorRed+"Erro CRÍTICO: Sala não encontrada para %s ao processar nota."+ColorReset, player.Login)
				return
			}
			...
			if gameEnded, _ := resultData["gameEnded"].(bool); gameEnded {
				...
				p1CoinsEarned := finalP1Score*5 + (func() int {
					if winner == p1Login {
						return 20
					}
					return 0
				}())
				p2CoinsEarned := finalP2Score*5 + (func() int {
					if winner == p2Login {
						return 20
					}
					return 0
	...
	for _, rarity := range rarities {
		for i := 0; i < 50; i++ {
			...
			if instrument != nil {
				...
				// Loop infinito até achar um ID que NÃO esteja no mapa
				for {
					uniqueID = randomGenerateDeterministic(localRand, 4)
					if _, exists := packetStock[uniqueID]; !exists {
						break // ID é virgem, pode sair do loop
					}
					// Se existir, o loop roda de novo e gera outro
				}
...
func startHttpApi(nodeID string, httpAddr string) {
	...
	mux.HandleFunc("/request-create-room", func(w http.ResponseWriter, r *http.Request) {
		...
		if playerLogin == "" {
			http.Error(w, "Missing player_login", http.StatusBadRequest)
			return
		}
	...
	mux.HandleFunc("/request-find-room", func(w http.ResponseWriter, r *http.Request) {
		...
		if mode == "PUBLIC" {
			...
			waitingRoomExists := len(salasEmEspera) > 0
			if waitingRoomExists {
				targetRoomID = salasEmEspera[0].ID
			}
			mu.Unlock()
			...
			if waitingRoomExists {
				...
				if fsmResp.response != "RoomCreated" {
					http.Error(w, fmt.Sprintf("Não foi possível criar sala: %s", fsmResp.response), 400)
					return
				}
	...
	mux.HandleFunc("/request-play-note", func(w http.ResponseWriter, r *http.Request) {
		...
		if playerLogin == "" || note == "" {
			http.Error(w, "Dados ausentes", http.StatusBadRequest)
			return
		}
		...
		case map[string]interface{}: // Sucesso
			...
			if salaID != "" {
				...
				if roundResultData, ok := resultData["roundResult"]; ok {
					...
					mapToStruct(roundResultData, &roundResult)
					resultMsgP1 := roundResult
					resultMsgP1.YourScore = currentP1Score
					resultMsgP1.OpponentScore = currentP2Score
					notifyPlayer(p1Login, protocolo.Message{Type: "ROUND_RESULT", Data: resultMsgP1})
					resultMsgP2 := roundResult
					resultMsgP2.YourScore = currentP2Score
					resultMsgP2.OpponentScore = currentP1Score
					notifyPlayer(p2Login, protocolo.Message{Type: "ROUND_RESULT", Data: resultMsgP2})
				}
...
func main() {
	...
	initBlockchain() // CHAMADA DA BLOCKCHAIN
